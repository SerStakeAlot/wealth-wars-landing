...
import type { LottoServices } from '../services/lotto/index.js';
import type { UserIdentityService } from '../services/user-identity.js';
import {
  validateBody,
  validateQuery,
  validateParams,
...
export function createLottoRoutes(
  ...
  const sendJson = (res: Response, data: any, status: number = 200) => {
  ...
  router.post(
    '/users/web',
    validateBody(CreateWebUserSchema),
  ...
  router.post(
    '/users/telegram',
    validateBody(CreateTelegramUserSchema),
    asyncHandler(async (req, res) => {
  ...
  router.post(
    '/rounds/:id/join/web',
    validateParams(z.object({ id: z.string().cuid() })),
    validateBody(JoinRoundWebSchema),
    asyncHandler(async (req, res) => {
   ...
   */
  router.post(
    '/rounds/:id/join/telegram',
    validateParams(z.object({ id: z.string().cuid() })),
    validateBody(JoinRoundTelegramSchema),
    asyncHandler(async (req, res) => {
      const { id: roundId } = req.params;
      const { telegramId, username } = req.body;
      ...
      if (!user.wallet) {
        return errorResponse(
          res,
          'Telegram user must link a wallet first',
          400
        );
      }
