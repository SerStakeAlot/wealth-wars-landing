...
export class EntryProcessor {
   ...
   */
  private async getNextEntryNonce(onchainRoundId: bigint): Promise<number> {
   ...
   */
  async joinRound(params: JoinRoundParams): Promise<EntryInfo> {
    ...
    const signature = await signAndSendTransaction(
      this.connection,
      tx,
      [this.authority] // In production, user would sign
    );
   ...
   */
  async getRoundEntries(roundId: string): Promise<EntryInfo[]> {
   ...
   */
  async getUserEntry(roundId: string, userId: string): Promise<EntryInfo | null> {
