...
app.post('/link/finish', async (req, res) => {
    ...
    if (!pending)
        return res.status(400).json({ error: 'No pending link' });
    try {
...
app.get('/wallet/:address/wealth', async (req, res) => {
    try {
        const { address } = req.params;
        if (!WEALTH_MINT)
            return res.status(500).json({ error: 'WEALTH_MINT not set' });
...
app.get('/healthz', (_, res) => res.json({ ok: true, cluster: CLUSTER }));
// Test route
