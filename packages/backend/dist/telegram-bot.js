...
⏰ Match begins when someone joins.`);
    ...
    bot.command('join', async (ctx) => {
        try {
            ...
            const existingEntry = round.entries.find((entry) => entry.userId === user.id);
            if (existingEntry) {
...
⏰ **30-second countdown begins now!**`);
            ...
            const countdownInterval = setInterval(async () => {
                ...
                if (timeLeft === 20 || timeLeft === 10 || timeLeft === 5 || timeLeft === 3 || timeLeft === 1) {
                    try {
                        await ctx.telegram.sendMessage(ctx.chat.id, `⏰ Match #${round.id} - ${timeLeft} seconds remaining!`);
                    }
            ...
            setTimeout(async () => {
                ...
                try {
                    ...
                    if (currentRound && currentRound.status === 'OPEN' && currentRound.entries.length === 2) {
                        ...
                        const totalPotLamports = toBigInt(currentRound.potAmount);
                        ...
                        // Announce winner
                        const entryAmount = bigintToLamports(winnerEntry.amount);
                        ...
                        await ctx.telegram.sendMessage(ctx.chat.id, `🎰 **TIME'S UP! Match #${round.id} Complete!**\n\n` +
